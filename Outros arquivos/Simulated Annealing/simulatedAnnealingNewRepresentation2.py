...
def createInitialSolution(n, g, L, U, p):
	...
	outerIterations = 0
	while((feasibleSolution == False) and outerIterations < n*n):
		solution = []
		...
		for j in range(0, g): #grupo atual que receberá vértices
			...
			solution.append([])
			innerIterations = 0
			while((groupWeight < L[j]) and innerIterations < n*n): #enquanto há tempo, tentamos botar vértices randomicamente no grupo, até atingirmos ou ultrapassarmos seu limite inferior
				vertexIndex = random.randint(0, n-1)
				innerInnerIterations = 0
				while vertexIndex in currentlyUsedVertices:
					if innerInnerIterations == n*n:
		...
		innerIterations = 0
		while((len(currentlyUsedVertices) < n) and innerIterations < n*n): # caso ainda existam vértices sem grupo, tentamos adicioná-los a um grupo
...
def createNeighbor(g, n, solution, L, U, p, kChange):
	...
	while numberOfChanges < kChange:
		vertexLeft = False
		while((vertexLeft == False) and (iterations < n*n)): #tenta criar um vizinho a partir de solution n vezes
...
parser.add_argument('-nsi', type=int, nargs=1, help='número de soluções iniciais a serem criadas pelo algoritmo; escolhe a melhor')
